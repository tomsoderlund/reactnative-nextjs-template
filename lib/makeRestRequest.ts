...
// export const fetcher = async (path: string): Promise<object> => await fetch(config.apiBaseUrl as string + path).then(async (res) => await res.json())
export const restFetcher = async (path: string): Promise<object> => {
  console.log('restFetcher URL:', path)
